 ...
 *   nozzle_line_width_mm, wall_count, infill_percent, top_bottom_layers_mm,
 *   support_type ("none"|"grid"|"tree"|"organic"),
 *   support_touches_part, overhang_fraction (0-1, rough est. of part needing support),
 *   interface_fraction (0-1, portion of support volume that should be interface material),
...
export function calcMaterialUsage(geometry, settings, materials) {
  ...
  return {
    ...
    usage: results,
    total_cost,
    note: "Estimate only -- not gcode-accurate. Assumes constant-thickness shell approximation and simplified support density factors.",
  };
}
...
export function checkBedFit(bbox, printer, shrinkagePercent = 100) {
  ...
  // Check all axis-order permutations since parts can be rotated on the bed
  const orientations = [
